// IEEE KEC Societies Detailed Data Structure
export const societiesData = {
  "student-branch": {
    name: "IEEE Student Branch",
    tagline: "Empowering Student Engineering Leadership and Global Professional Collaboration",
    theme: {
      primary: "#00629B", // IEEE Blue
      secondary: "#ffffff"
    },
    heroImage: "https://images.unsplash.com/photo-1540575467063-178a50c2df87?q=80&w=1200",
    logoText: "IEEE KEC SB",
    stats: {
      members: "150+",
      events: "45+",
      awards: "12+",
      projects: "25+"
    },
    about: {
      overview: "The IEEE Kongu Engineering College Student Branch (IEEE KEC SB) is the umbrella student body facilitating technical learning, project developments, and leadership building. We foster a collaborative ecosystem where students gain exposure to globally recognized research, standards, and technical networks.",
      mission: "To inspire, enable, and foster professional and technical excellence among student members to engineer solutions for the benefit of local and global communities.",
      vision: "To become a prime hub of technical innovation and leadership in the region, bridging the gap between academic theories and industry expectations.",
      objectives: [
        "Nurture student research capabilities and prototype development.",
        "Facilitate active networking with global experts, researchers, and professional alumni.",
        "Promote technical awareness and hands-on exposure to advanced tools.",
        "Encourage leadership, volunteerism, and social responsibility."
      ]
    },
    facultyIncharge: {
      name: "Dr. A. Kumar",
      position: "Student Branch Counselor",
      department: "Electronics and Communication Engineering",
      phone: "+91 94435 12345",
      email: "advisor@kec.edu",
      photo: "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?q=80&w=300"
    },
    chairman: {
      name: "Abhishek M.",
      year: "IV Year",
      department: "Computer Science and Engineering",
      photo: "https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?q=80&w=300"
    },
    viceChairman: {
      name: "Sneha R.",
      year: "IV Year",
      department: "Electronics and Communication Engineering",
      photo: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=300"
    },
    officeBearers: [
      {
        name: "Harish K.",
        position: "Secretary",
        year: "III Year",
        department: "Electronics and Instrumentation Engineering",
        photo: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=300"
      },
      {
        name: "Naveen S.",
        position: "Joint Secretary",
        year: "III Year",
        department: "Mechanical Engineering",
        photo: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=300"
      },
      {
        name: "Divya K.",
        position: "Treasurer",
        year: "III Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=300"
      },
      {
        name: "Dharini P.",
        position: "Webmaster",
        year: "III Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=300"
      },
      {
        name: "Karthick M.",
        position: "Technical Coordinator",
        year: "III Year",
        department: "Electrical and Electronics Engineering",
        photo: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=300"
      },
      {
        name: "Arun Kumar S.",
        position: "Event Coordinator",
        year: "III Year",
        department: "Chemical Engineering",
        photo: "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?q=80&w=300"
      },
      {
        name: "Keerthana B.",
        position: "Membership Coordinator",
        year: "III Year",
        department: "Civil Engineering",
        photo: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=300"
      },
      {
        name: "Sanjay R.",
        position: "Design Lead",
        year: "III Year",
        department: "Mechatronics Engineering",
        photo: "https://images.unsplash.com/photo-1501196354995-cbb51c65aaea?q=80&w=300"
      }
    ],
    members: [
      {
        name: "Ramprasath T.",
        year: "II Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1522075469751-3a6694fb2f61?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Deepika S.",
        year: "II Year",
        department: "Electronics and Communication Engineering",
        photo: "https://images.unsplash.com/photo-1580489944761-15a19d654956?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Vignesh A.",
        year: "II Year",
        department: "Electrical and Electronics Engineering",
        photo: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Megala R.",
        year: "II Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      }
    ],
    gallery: [
      { url: "https://images.unsplash.com/photo-1515187029135-18ee286d815b?q=80&w=600", caption: "Inauguration Ceremony" },
      { url: "https://images.unsplash.com/photo-1531482615713-2afd69097998?q=80&w=600", caption: "National Level Workshop" },
      { url: "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?q=80&w=600", caption: "24-Hour Hackathon" },
      { url: "https://images.unsplash.com/photo-1528605248644-14dd04022da1?q=80&w=600", caption: "Executive Committee Meeting" }
    ],
    contact: {
      email: "ieee@kec.edu",
      phone: "+91 4294 226555",
      location: "IEEE Student Branch Cabin, Main Block, Kongu Engineering College, Perundurai, Erode - 638060",
      socials: {
        linkedin: "https://linkedin.com",
        instagram: "https://instagram.com",
        facebook: "https://facebook.com",
        ieee: "https://ieee.org"
      }
    },
    milestones: [
      {
        title: "Madras Section Best Branch Award",
        date: "2025",
        description: "Recognized as one of the most active and high-performing student branches under the Madras Section for technical and community contributions.",
        image: "https://images.unsplash.com/photo-1531545514256-b1400bc00f31?q=80&w=400"
      },
      {
        title: "All India Student Congress",
        date: "2024",
        description: "Successfully hosted delegation for the annual IEEE All India Student Congress with 200+ regional student leaders and guest speakers.",
        image: "https://images.unsplash.com/photo-1475721027785-f74eccf877e2?q=80&w=400"
      }
    ]
  },
  "computer-society": {
    name: "IEEE Computer Society",
    tagline: "Advancing computing technologies to empower innovation and create impact.",
    theme: {
      primary: "#003366", // Dark Blue
      secondary: "#00BCD4" // Cyan
    },
    heroImage: "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?q=80&w=1200",
    logoText: "IEEE CS",
    stats: {
      members: "120+",
      events: "35+",
      awards: "15+",
      projects: "20+"
    },
    about: {
      overview: "The IEEE Computer Society (CS) is the premier source for information, inspiration, and collaboration in computer science and engineering. We provide computer engineers and scientists with resources to accelerate their careers, push the boundaries of technical research, and foster computing-centered student innovation.",
      mission: "To advance the theory, design, practice, and application of computer and information processing science and technology, and to promote professional standing.",
      vision: "To be the leading global provider of technical information and services to computing professionals and student developers.",
      objectives: [
        "Equip students with high-demand skills in software development, machine learning, and cybersecurity.",
        "Facilitate research and project guidance under experienced faculty coordinators.",
        "Conduct competitive events, coding sprints, and project exhibitions.",
        "Bridge academia and IT giants through technical talk shows and webinars."
      ]
    },
    facultyIncharge: {
      name: "Dr. S. Varadhaganapathy",
      position: "Society Advisor",
      department: "Computer Science and Engineering",
      phone: "+91 98427 21111",
      email: "varadhaganapathy@kec.edu",
      photo: "https://images.unsplash.com/photo-1560250097-0b93528c311a?q=80&w=300"
    },
    chairman: {
      name: "Karthik Raja V.",
      year: "IV Year",
      department: "Computer Science and Engineering",
      photo: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=300"
    },
    viceChairman: {
      name: "Divya Bharathi K.",
      year: "IV Year",
      department: "Information Technology",
      photo: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=300"
    },
    officeBearers: [
      {
        name: "Sudharsan R.",
        position: "Secretary",
        year: "III Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=300"
      },
      {
        name: "Preethi M.",
        position: "Joint Secretary",
        year: "III Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=300"
      },
      {
        name: "Logeshwaran P.",
        position: "Treasurer",
        year: "III Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=300"
      },
      {
        name: "Prakash R.",
        position: "Webmaster",
        year: "III Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?q=80&w=300"
      },
      {
        name: "Shruthi V.",
        position: "Technical Coordinator",
        year: "III Year",
        department: "Artificial Intelligence & Data Science",
        photo: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=300"
      },
      {
        name: "Vikas G.",
        position: "Event Coordinator",
        year: "III Year",
        department: "Computer Science and Design",
        photo: "https://images.unsplash.com/photo-1501196354995-cbb51c65aaea?q=80&w=300"
      },
      {
        name: "Manoj K.",
        position: "Membership Coordinator",
        year: "III Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1522075469751-3a6694fb2f61?q=80&w=300"
      },
      {
        name: "Karthika D.",
        position: "Design Lead",
        year: "III Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=300"
      }
    ],
    members: [
      {
        name: "Arun Bose S.",
        year: "II Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Shalini S.",
        year: "II Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1580489944761-15a19d654956?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Dinesh K.",
        year: "II Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1522075469751-3a6694fb2f61?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Pavithra R.",
        year: "II Year",
        department: "Artificial Intelligence & Machine Learning",
        photo: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      }
    ],
    gallery: [
      { url: "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?q=80&w=600", caption: "CodeSprint Hackathon" },
      { url: "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?q=80&w=600", caption: "Web Tech Training" },
      { url: "https://images.unsplash.com/photo-1531482615713-2afd69097998?q=80&w=600", caption: "Machine Learning Workshop" },
      { url: "https://images.unsplash.com/photo-1515187029135-18ee286d815b?q=80&w=600", caption: "Guest Talk on Cloud Computing" }
    ],
    contact: {
      email: "computer.society@kec.edu",
      phone: "+91 98427 21111",
      location: "Systems Lab, Dept of CSE, Kongu Engineering College, Perundurai, Erode - 638060",
      socials: {
        linkedin: "https://linkedin.com",
        instagram: "https://instagram.com",
        facebook: "https://facebook.com",
        ieee: "https://computer.org"
      }
    },
    milestones: [
      {
        title: "Outstanding Chapter Award",
        date: "2025",
        description: "Honored with the Outstanding Student Chapter Award for exceptional technical workshops, research symposiums, and coding hackathons.",
        image: "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?q=80&w=400"
      },
      {
        title: "Hackathon Excellence",
        date: "2024",
        description: "Ranked among the Top 5 regional student chapters for competitive coding achievements and student developer incubator programs.",
        image: "https://images.unsplash.com/photo-1515187029135-18ee286d815b?q=80&w=400"
      }
    ]
  },
  "wie": {
    name: "IEEE Women in Engineering (WIE)",
    tagline: "Inspiring, engaging and advancing women in technology worldwide.",
    theme: {
      primary: "#8E44AD", // Purple
      secondary: "#E91E63" // Pink
    },
    heroImage: "https://images.unsplash.com/photo-1573164713988-8665fc963095?q=80&w=1200",
    logoText: "IEEE WIE",
    stats: {
      members: "110+",
      events: "30+",
      awards: "10+",
      projects: "15+"
    },
    about: {
      overview: "IEEE Women in Engineering (WIE) is a global network of IEEE members and volunteers dedicated to promoting women engineers and scientists, and inspiring girls around the world to follow their academic interests in a career in engineering and science.",
      mission: "To inspire, engage, encourage, and empower IEEE women in engineering and science globally.",
      vision: "A vibrant community of IEEE women and men collectively using their diverse talents to innovate for the benefit of humanity.",
      objectives: [
        "Provide mentorship and professional growth paths for female engineering students.",
        "Organize coding and technology bootcamps tailored for female developers.",
        "Facilitate leadership workshops and outreach programs in local girls' schools.",
        "Reward academic and volunteer achievements through WIE recognition awards."
      ]
    },
    facultyIncharge: {
      name: "Dr. J. Premalatha",
      position: "Affinity Group Advisor",
      department: "Information Technology",
      phone: "+91 98427 25555",
      email: "premalatha@kec.edu",
      photo: "https://images.unsplash.com/photo-1573497019940-1c28c88b4f3e?q=80&w=300"
    },
    chairman: {
      name: "Priyanka S.",
      year: "IV Year",
      department: "Information Technology",
      photo: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=300"
    },
    viceChairman: {
      name: "Keerthana M.",
      year: "IV Year",
      department: "Electronics and Communication Engineering",
      photo: "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=300"
    },
    officeBearers: [
      {
        name: "Abirami R.",
        position: "Secretary",
        year: "III Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=300"
      },
      {
        name: "Subashini G.",
        position: "Joint Secretary",
        year: "III Year",
        department: "Electrical and Electronics Engineering",
        photo: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=300"
      },
      {
        name: "Soundarya K.",
        position: "Treasurer",
        year: "III Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1580489944761-15a19d654956?q=80&w=300"
      },
      {
        name: "Nithya R.",
        position: "Webmaster",
        year: "III Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=300"
      },
      {
        name: "Sandhiya S.",
        position: "Technical Coordinator",
        year: "III Year",
        department: "Electronics and Communication Engineering",
        photo: "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=300"
      },
      {
        name: "Mythili T.",
        position: "Event Coordinator",
        year: "III Year",
        department: "Food Technology",
        photo: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=300"
      },
      {
        name: "Pavithra M.",
        position: "Membership Coordinator",
        year: "III Year",
        department: "Chemical Engineering",
        photo: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=300"
      },
      {
        name: "Shruthi K.",
        position: "Design Lead",
        year: "III Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=300"
      }
    ],
    members: [
      {
        name: "Gowsika V.",
        year: "II Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1580489944761-15a19d654956?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Harini M.",
        year: "II Year",
        department: "Electronics and Instrumentation Engineering",
        photo: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Roshini B.",
        year: "II Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Divya A.",
        year: "II Year",
        department: "Civil Engineering",
        photo: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      }
    ],
    gallery: [
      { url: "https://images.unsplash.com/photo-1573164713988-8665fc963095?q=80&w=600", caption: "Women Techmakers Summit" },
      { url: "https://images.unsplash.com/photo-1515187029135-18ee286d815b?q=80&w=600", caption: "Coding Bootcamp for Girls" },
      { url: "https://images.unsplash.com/photo-1531482615713-2afd69097998?q=80&w=600", caption: "Outreach Program at Government School" },
      { url: "https://images.unsplash.com/photo-1528605248644-14dd04022da1?q=80&w=600", caption: "Annual Leadership Conclave" }
    ],
    contact: {
      email: "wie@kec.edu",
      phone: "+91 98427 25555",
      location: "WIE Cabin, IT Dept, Kongu Engineering College, Perundurai, Erode - 638060",
      socials: {
        linkedin: "https://linkedin.com",
        instagram: "https://instagram.com",
        facebook: "https://facebook.com",
        ieee: "https://wie.ieee.org"
      }
    },
    milestones: [
      {
        title: "Inspiring Women Leaders Summit",
        date: "2025",
        description: "Hosted a regional symposium empowering female engineers with tech panels, workshops, and startup funding mentorship.",
        image: "https://images.unsplash.com/photo-1573497019940-1c28c88b4f3e?q=80&w=400"
      }
    ]
  },
  "ras": {
    name: "IEEE Robotics and Automation Society (RAS)",
    tagline: "Driving innovation in robotics and intelligent automation systems.",
    theme: {
      primary: "#FF6F00", // Orange
      secondary: "#263238" // Dark Gray
    },
    heroImage: "https://images.unsplash.com/photo-1485827404703-89b55fcc595e?q=80&w=1200",
    logoText: "IEEE RAS",
    stats: {
      members: "100+",
      events: "28+",
      awards: "8+",
      projects: "18+"
    },
    about: {
      overview: "The IEEE Robotics and Automation Society (RAS) is active in hosting project hackathons, robot fabrication events, and IoT automation setups. We bridge electrical, mechanical, and software disciplines to enable students to design robust, autonomous machines.",
      mission: "To foster the advancement, development, and integration of robotics and automation engineering theories and practices.",
      vision: "To become a pioneering student society driving real-world automation solutions and robotic research.",
      objectives: [
        "Train students in ROS (Robot Operating System), Arduino, Raspberry Pi, and mechanical CAD design.",
        "Fund student teams representing KEC in national robotics challenges.",
        "Collaborate with industry automation research labs for mentorship.",
        "Conduct robo-soccer, line follower, and drone racing events."
      ]
    },
    facultyIncharge: {
      name: "Dr. R. Murugesan",
      position: "Society Advisor",
      department: "Mechatronics Engineering",
      phone: "+91 98427 23333",
      email: "murugesan@kec.edu",
      photo: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?q=80&w=300"
    },
    chairman: {
      name: "Jegadeeshwaran S.",
      year: "IV Year",
      department: "Mechatronics Engineering",
      photo: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=300"
    },
    viceChairman: {
      name: "Kavya R.",
      year: "IV Year",
      department: "Electronics and Communication Engineering",
      photo: "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=300"
    },
    officeBearers: [
      {
        name: "Arun Kumar B.",
        position: "Secretary",
        year: "III Year",
        department: "Mechatronics Engineering",
        photo: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=300"
      },
      {
        name: "Praveen V.",
        position: "Joint Secretary",
        year: "III Year",
        department: "Electrical and Electronics Engineering",
        photo: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=300"
      },
      {
        name: "Keerthika R.",
        position: "Treasurer",
        year: "III Year",
        department: "Mechatronics Engineering",
        photo: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=300"
      },
      {
        name: "Ranjan M.",
        position: "Webmaster",
        year: "III Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1522075469751-3a6694fb2f61?q=80&w=300"
      },
      {
        name: "Santhosh K.",
        position: "Technical Coordinator",
        year: "III Year",
        department: "Mechatronics Engineering",
        photo: "https://images.unsplash.com/photo-1501196354995-cbb51c65aaea?q=80&w=300"
      },
      {
        name: "Ganesh R.",
        position: "Event Coordinator",
        year: "III Year",
        department: "Mechanical Engineering",
        photo: "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?q=80&w=300"
      },
      {
        name: "Devaki S.",
        position: "Membership Coordinator",
        year: "III Year",
        department: "Electronics and Instrumentation Engineering",
        photo: "https://images.unsplash.com/photo-1580489944761-15a19d654956?q=80&w=300"
      },
      {
        name: "Raghav J.",
        position: "Design Lead",
        year: "III Year",
        department: "Mechatronics Engineering",
        photo: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=300"
      }
    ],
    members: [
      {
        name: "Gokul M.",
        year: "II Year",
        department: "Mechatronics Engineering",
        photo: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Abinaya K.",
        year: "II Year",
        department: "Electronics and Communication Engineering",
        photo: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Saran Kumar P.",
        year: "II Year",
        department: "Mechanical Engineering",
        photo: "https://images.unsplash.com/photo-1522075469751-3a6694fb2f61?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Pavithra J.",
        year: "II Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      }
    ],
    gallery: [
      { url: "https://images.unsplash.com/photo-1485827404703-89b55fcc595e?q=80&w=600", caption: "Robo-Soccer Competition" },
      { url: "https://images.unsplash.com/photo-1581092334651-ddf26d9a09d0?q=80&w=600", caption: "PCB Design and Fabrication Workshop" },
      { url: "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?q=80&w=600", caption: "Line Follower Robot Challenge" },
      { url: "https://images.unsplash.com/photo-1515187029135-18ee286d815b?q=80&w=600", caption: "ROS Hands-on Training" }
    ],
    contact: {
      email: "ras@kec.edu",
      phone: "+91 98427 23333",
      location: "Robotics and Automation Lab, Mechatronics Dept, Kongu Engineering College, Perundurai, Erode - 638060",
      socials: {
        linkedin: "https://linkedin.com",
        instagram: "https://instagram.com",
        facebook: "https://facebook.com",
        ieee: "https://ieee-ras.org"
      }
    },
    milestones: [
      {
        title: "RoboFest Championship",
        date: "2025",
        description: "Students secured 1st prize in the national level automated line follower and rescue bot design competition.",
        image: "https://images.unsplash.com/photo-1485827404703-89b55fcc595e?q=80&w=400"
      }
    ]
  },
  "pes": {
    name: "IEEE Power and Energy Society (PES)",
    tagline: "Powering the future through sustainable energy solutions.",
    theme: {
      primary: "#2E7D32", // Green
      secondary: "#81C784" // Light Green
    },
    heroImage: "https://images.unsplash.com/photo-1473341304170-971dccb5ac1e?q=80&w=1200",
    logoText: "IEEE PES",
    stats: {
      members: "95+",
      events: "25+",
      awards: "7+",
      projects: "12+"
    },
    about: {
      overview: "The IEEE Power & Energy Society (PES) is the leading provider of scientific and engineering information on electric power and energy. KEC PES Student Branch Chapter drives research and implementation of solar, wind, smart grid, electric vehicle, and microgrid engineering.",
      mission: "To be the leading provider of scientific and engineering information on electric power and energy for the betterment of society.",
      vision: "A world where clean, reliable, and sustainable power and energy are universally accessible.",
      objectives: [
        "Introduce student members to solar-grid design, battery management systems, and smart grid automation.",
        "Support student design projects targeting electric vehicle configurations and energy harvesters.",
        "Conduct symposiums and site visits to thermal, wind, and hydro stations.",
        "Foster networking with global energy research organizations."
      ]
    },
    facultyIncharge: {
      name: "Dr. N. Nithyadevi",
      position: "Society Advisor",
      department: "Electrical and Electronics Engineering",
      phone: "+91 98427 27777",
      email: "nithyadevi@kec.edu",
      photo: "https://images.unsplash.com/photo-1580489944761-15a19d654956?q=80&w=300"
    },
    chairman: {
      name: "Hariharan R.",
      year: "IV Year",
      department: "Electrical and Electronics Engineering",
      photo: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=300"
    },
    viceChairman: {
      name: "Deepa K.",
      year: "IV Year",
      department: "Electrical and Electronics Engineering",
      photo: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=300"
    },
    officeBearers: [
      {
        name: "Arun M.",
        position: "Secretary",
        year: "III Year",
        department: "Electrical and Electronics Engineering",
        photo: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=300"
      },
      {
        name: "Gayathri P.",
        position: "Joint Secretary",
        year: "III Year",
        department: "Electronics and Instrumentation Engineering",
        photo: "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=300"
      },
      {
        name: "Dinesh Kumar T.",
        position: "Treasurer",
        year: "III Year",
        department: "Electrical and Electronics Engineering",
        photo: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=300"
      },
      {
        name: "Indhumathi R.",
        position: "Webmaster",
        year: "III Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1580489944761-15a19d654956?q=80&w=300"
      },
      {
        name: "Sanjay Bose M.",
        position: "Technical Coordinator",
        year: "III Year",
        department: "Electrical and Electronics Engineering",
        photo: "https://images.unsplash.com/photo-1522075469751-3a6694fb2f61?q=80&w=300"
      },
      {
        name: "Pavithran S.",
        position: "Event Coordinator",
        year: "III Year",
        department: "Chemical Engineering",
        photo: "https://images.unsplash.com/photo-1501196354995-cbb51c65aaea?q=80&w=300"
      },
      {
        name: "Abirami B.",
        position: "Membership Coordinator",
        year: "III Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=300"
      },
      {
        name: "Vigneshwaran K.",
        position: "Design Lead",
        year: "III Year",
        department: "Electrical and Electronics Engineering",
        photo: "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?q=80&w=300"
      }
    ],
    members: [
      {
        name: "Balaji V.",
        year: "II Year",
        department: "Electrical and Electronics Engineering",
        photo: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Yamuna S.",
        year: "II Year",
        department: "Electrical and Electronics Engineering",
        photo: "https://images.unsplash.com/photo-1580489944761-15a19d654956?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Mohammad Anas J.",
        year: "II Year",
        department: "Electronics and Instrumentation Engineering",
        photo: "https://images.unsplash.com/photo-1522075469751-3a6694fb2f61?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Monisha R.",
        year: "II Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      }
    ],
    gallery: [
      { url: "https://images.unsplash.com/photo-1473341304170-971dccb5ac1e?q=80&w=600", caption: "Windmill Field Site Visit" },
      { url: "https://images.unsplash.com/photo-1515187029135-18ee286d815b?q=80&w=600", caption: "EV Charging Station Presentation" },
      { url: "https://images.unsplash.com/photo-1531482615713-2afd69097998?q=80&w=600", caption: "Smart Grid Training Session" },
      { url: "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?q=80&w=600", caption: "Renewable Energy Project Expo" }
    ],
    contact: {
      email: "pes@kec.edu",
      phone: "+91 98427 27777",
      location: "Electrical Machines Lab, Dept of EEE, Kongu Engineering College, Perundurai, Erode - 638060",
      socials: {
        linkedin: "https://linkedin.com",
        instagram: "https://instagram.com",
        facebook: "https://facebook.com",
        ieee: "https://ieee-pes.org"
      }
    },
    milestones: [
      {
        title: "Smart Grid Design Hackathon",
        date: "2025",
        description: "Students designed modular energy management systems for urban micro-grids, securing Anna University innovation grant.",
        image: "https://images.unsplash.com/photo-1473341304170-971dccb5ac1e?q=80&w=400"
      }
    ]
  },
  "comsoc": {
    name: "IEEE Communications Society (ComSoc)",
    tagline: "Connecting the world through communication technologies.",
    theme: {
      primary: "#00897B", // Teal
      secondary: "#2196F3" // Blue
    },
    heroImage: "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?q=80&w=1200",
    logoText: "IEEE ComSoc",
    stats: {
      members: "105+",
      events: "32+",
      awards: "9+",
      projects: "14+"
    },
    about: {
      overview: "The IEEE Communications Society (ComSoc) promotes technological innovation and fosters the creation and sharing of information among global communication engineers. Our KEC chapter targets 5G/6G systems, optical grids, wireless networks, and RF antenna designs.",
      mission: "To foster technical information exchange and professional development in communications engineering and science.",
      vision: "To connect people, machines, and networks globally to build a smarter, safer technical future.",
      objectives: [
        "Expose students to RF simulations, optical fiber splices, and cellular base station configurations.",
        "Encourage research publications in peer-reviewed communications journals.",
        "Conduct IoT connectivity and cloud integration coding challenges.",
        "Organize workshops on modern network protocols and network security."
      ]
    },
    facultyIncharge: {
      name: "Dr. K. Senthil Kumar",
      position: "Society Advisor",
      department: "Electronics and Communication Engineering",
      phone: "+91 98427 29999",
      email: "senthilkumar@kec.edu",
      photo: "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?q=80&w=300"
    },
    chairman: {
      name: "Siddharth K.",
      year: "IV Year",
      department: "Electronics and Communication Engineering",
      photo: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=300"
    },
    viceChairman: {
      name: "Akshara R.",
      year: "IV Year",
      department: "Electronics and Communication Engineering",
      photo: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=300"
    },
    officeBearers: [
      {
        name: "Mohanraj R.",
        position: "Secretary",
        year: "III Year",
        department: "Electronics and Communication Engineering",
        photo: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=300"
      },
      {
        name: "Abinaya S.",
        position: "Joint Secretary",
        year: "III Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=300"
      },
      {
        name: "Rajesh K.",
        position: "Treasurer",
        year: "III Year",
        department: "Electronics and Communication Engineering",
        photo: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=300"
      },
      {
        name: "Pooja V.",
        position: "Webmaster",
        year: "III Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1580489944761-15a19d654956?q=80&w=300"
      },
      {
        name: "Pranesh J.",
        position: "Technical Coordinator",
        year: "III Year",
        department: "Electronics and Communication Engineering",
        photo: "https://images.unsplash.com/photo-1522075469751-3a6694fb2f61?q=80&w=300"
      },
      {
        name: "Harish S.",
        position: "Event Coordinator",
        year: "III Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1501196354995-cbb51c65aaea?q=80&w=300"
      },
      {
        name: "Janani K.",
        position: "Membership Coordinator",
        year: "III Year",
        department: "Electronics and Communication Engineering",
        photo: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=300"
      },
      {
        name: "Sanjay Kumar M.",
        position: "Design Lead",
        year: "III Year",
        department: "Electronics and Communication Engineering",
        photo: "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?q=80&w=300"
      }
    ],
    members: [
      {
        name: "Naveenkumar B.",
        year: "II Year",
        department: "Electronics and Communication Engineering",
        photo: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Nithyasree G.",
        year: "II Year",
        department: "Electronics and Communication Engineering",
        photo: "https://images.unsplash.com/photo-1580489944761-15a19d654956?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Balakrishnan S.",
        year: "II Year",
        department: "Computer Science and Engineering",
        photo: "https://images.unsplash.com/photo-1522075469751-3a6694fb2f61?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      },
      {
        name: "Bhuvaneswari R.",
        year: "II Year",
        department: "Information Technology",
        photo: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=300",
        socials: { linkedin: "#", instagram: "#", facebook: "#", ieee: "#" }
      }
    ],
    gallery: [
      { url: "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?q=80&w=600", caption: "5G Networking Seminar" },
      { url: "https://images.unsplash.com/photo-1531482615713-2afd69097998?q=80&w=600", caption: "RF Antenna Design and Simulation" },
      { url: "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?q=80&w=600", caption: "IoT Connectivity Hackathon" },
      { url: "https://images.unsplash.com/photo-1515187029135-18ee286d815b?q=80&w=600", caption: "Optical Fiber Fusion Splicing training" }
    ],
    contact: {
      email: "comsoc@kec.edu",
      phone: "+91 98427 29999",
      location: "Microwave and Fiber Optics Lab, Dept of ECE, Kongu Engineering College, Perundurai, Erode - 638060",
      socials: {
        linkedin: "https://linkedin.com",
        instagram: "https://instagram.com",
        facebook: "https://facebook.com",
        ieee: "https://comsoc.org"
      }
    },
    milestones: [
      {
        title: "Telecommunication Prototype Grant",
        date: "2025",
        description: "Received research funding for building open-source LoRaWAN mesh communication systems for disaster rescue.",
        image: "https://images.unsplash.com/photo-1544256718-3bcf237f3974?q=80&w=400"
      }
    ]
  }
};
