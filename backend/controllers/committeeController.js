import Committee from '../models/Committee.js';

export const getCommittees = async (req, res) => {
  try { const items = await Committee.find({}); res.json(items); }
  catch (error) { res.status(500).json({ message: error.message }); }
};

export const createCommittee = async (req, res) => {
  try { const item = new Committee(req.body); const created = await item.save(); res.status(201).json(created); }
  catch (error) { res.status(400).json({ message: error.message }); }
};

export const updateCommittee = async (req, res) => {
  try { const item = await Committee.findOneAndUpdate({ id: req.params.id }, req.body, {new:true});
    if (item) res.json(item); else res.status(404).json({ message: 'Not found' });
  } catch (error) { res.status(400).json({ message: error.message }); }
};

export const deleteCommittee = async (req, res) => {
  try { const item = await Committee.findOneAndDelete({ id: req.params.id });
    if (item) res.json({ message: 'Removed' }); else res.status(404).json({ message: 'Not found' });
  } catch (error) { res.status(500).json({ message: error.message }); }
};